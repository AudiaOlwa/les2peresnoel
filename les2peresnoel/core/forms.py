...
SECTOR_DETAIL_CHOICES = [
    ...
    (
        "POSTAL_OPERATOR",
        "Opérateur services postaux (création bureau de poste PNS – timbres – enveloppe – etc.)",
    ),
...
class SignUpForm(forms.Form):
    ...
    utilisateur = forms.ChoiceField(
        choices=USER_TYPE_CHOICES, widget=forms.Select(attrs={"class": "form-control"})
    )
    secteur = forms.ChoiceField(
        choices=SECTOR_CHOICES, widget=forms.Select(attrs={"class": "form-control"})
    )
    detail_secteur = forms.ChoiceField(
        choices=SECTOR_DETAIL_CHOICES,
